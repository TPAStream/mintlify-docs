---
title: Client Usage
description: We have decided that the best possible way to implement this SDK is a simple config pattern. A prospective user will communicate with us to recieve an SDK apiToken as well as employer system keys.
---
...
- `user`
  ...
  - `policy_holders` -- This will list out all of the policy_holders associated with the user. You can think of these as the saved credentials per carrier. Data passed back here will be limited.
    ...
    - `id` and `policy_holder_id` -- The unique identifier TPAStream uses for this set of credentials. If you implement any of the _TPAStream Webhooks_ you will find this info useful for association.
- `tenant`
  - `id` and `tenant_id`
  - `name`
...
`doneEasyEnroll` This is the final callback of the SDK widget and it occurs when the system. There is a bunch of data dumped at the end which can be useful for an implementor when they are working the _TPAStream Webhooks_
