---
title: FAQ
description: Here we attempt to address several of the common questions which might pop-up while an implementor is working with the StreamConnect SDK
---
...
For more information on configuration objects see our [Client Usage Docs](client-usage).
...
See our [React Native Docs](./sdk-hook)

## I'd like to create a more customized flow for how a user enters credentials and validation.

The default recommendation for customizing the SDK flow is to use the various sdk [Callbacks](client-usage#callbacks).
...
Each of the provided information is present in the [Callbacks](client-usage#callbacks) docs.
...
If you are implementing the whole Stream Connect flow, including interfacing with the TPAStream API and using TPAStream webhooks, you will find the [doneEasyEnroll](client-usage#doneeasyenroll) callback to hold the main pieces of information you need.
...
- A `login_problem` which will define the state of the credentials. See [doneEasyEnroll](client-usage#doneeasyenroll)
...
Within the TPAStream configuration examples as well as the [Client Usage Docs](client-usage) you might sometimes see
...
See our [Client Usage Docs](client-usage) for more information on the `user` object block.
